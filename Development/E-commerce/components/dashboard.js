...
loadProducts()
    .then(products => {
        ...
        products.forEach(product => {
            // Check if product is in wishlist for heart icon
            const isFavorited = favorites.includes(product.id);
            // Set heart icon class (filled if favorited, empty otherwise)
            ...
            // Use red bold text for price if discounted, else normal bold
            const priceClass = product.originalPrice ? 'text-danger fw-bold' : 'fw-bold';
            // Generate color radio buttons if product has colors
            const colors = generateColors(product);
            // Consistent image styling for all products
            const imgStyle = 'style="height:220px;object-fit:contain;"';
            // Consistent image class for all products
            const imgClass = 'card-img-top bg-white p-4';
            // removed the discount badge and New tag mannualy
            const card = `
